...
static void from_char(std::string const& literal) {
	...
	print_table(static_cast<int>(input), static_cast<float>(input), static_cast<double>(input));
}

static void from_int(std::string const& literal) {
	const long wrap_input = static_cast<long>(std::atol(literal.c_str()));

	print_table(wrap_input, static_cast<float>(wrap_input), static_cast<double>(wrap_input));
}

static void from_float(std::string const& literal) {
	const float input = static_cast<float>(std::atof(literal.c_str()));

	print_table(static_cast<long>(input), static_cast<float>(input), static_cast<double>(input),
				is_pseudo_float(literal));
}

static void from_double(std::string const& literal) {
	const double input = static_cast<double>(std::strtod(literal.c_str(), NULL));

	print_table(static_cast<long>(input), static_cast<float>(input), input,
...
static FuncPtr from_type(std::string const& literal) {
	...
	if ((idx_dot != std::string::npos
		 && static_cast<std::string>(&literal.at(idx_dot)).find(".", 1) != std::string::npos)
		|| (idx_f != std::string::npos && idx_f != literal.length() - 1
			&& static_cast<std::string>(&literal.at(idx_f)).find("f") != std::string::npos
			&& literal != "+inff" && literal != "-inff")
		|| literal.find_first_of("-+") != literal.find_last_of("-+")) {
		return (&from_invalid);
	}
	if (is_pseudo_float(literal)
		|| (idx_dot != std::string::npos && literal.end()[-1] == 'f'
			&& std::isdigit(literal.end()[-2]))) {
		return (&from_float);
	}
	if (is_pseudo_double(literal)
		|| (idx_dot != std::string::npos && std::isdigit(literal.end()[-1])
...
void ScalarConverter::convert(std::string const& literal) {
	if ((literal.length() == 1 || (literal.length() > 1 && !contains_chars(literal)))
